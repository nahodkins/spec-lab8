...
public class MyStackImproved<T> {
    ...
    public MyStackImproved() {
    }

    public MyStackImproved(T value) {
    ...
    public void addAll(MyStackImproved<? extends T> stack) {
        MyStackImproved<T> tmp = (MyStackImproved<T>) stack;
        while (tmp.size != 0) {
            push(tmp.pop());
        }
    }

    public void moveElementsTo(MyStackImproved<? super T> stack) {
        MyStackImproved<T> tmp = this;
        while (tmp.size != 0) {
            stack.push(tmp.pop());
        }
    }
    ...
    public boolean equals(Object obj) {
        if (obj.getClass() != MyStackImproved.class) {
            return false;
        }
        MyStackImproved<T> other = (MyStackImproved<T>) obj;
        if (size != other.size) {
            return false;
        }
        boolean result = true;
        StackNode<T> thisPointer = topElement;
        StackNode<T> otherPointer = other.topElement;
        while (thisPointer != null) {
            result &= thisPointer.getValue().equals(otherPointer.getValue());
